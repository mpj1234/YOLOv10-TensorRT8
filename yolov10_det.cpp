...
void serialize_engine(std::string &wts_name, std::string &engine_name, std::string &type, float &gd,
...
int main(int argc, char **argv) {
    ...
    if (!parse_args(argc, argv, wts_name, engine_name, img_dir, type, gd, gw,
                    max_channels)) {
        std::cerr << "Arguments not right!" << std::endl;
        std::cerr << "./yolov10_det -s [.wts] [.engine] [n/s/m/b/l/x]  // serialize model to "
                     "plan file"
                  << std::endl;
        std::cerr << "./yolov10_det -d [.engine] ../samples  // deserialize plan file and run inference"
                  << std::endl;
        return -1;
    }
